...
{
    public static AlacContext AlacOpenFileInput(java.io.InputStream is)
    {
		int headerRead;
		...
		try
		{
			...
			else if(headerRead == 3)
			{
				...
				{
					is.reset();
					System.out.println("qtmovie.saved_mdat_pos:" + qtmovie.saved_mdat_pos);
					is.skip(qtmovie.saved_mdat_pos);
				} 
				catch (IOException e) 
				{
					System.out.println("ALAC not supported");
				}
			}
		}
		finally
		{
			is.mark(0);//remove mark
		}
